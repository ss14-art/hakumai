...
ent-ComputerGridControl = консоль управления сеткой
    .desc = Используется для предотвращения потери прав собственности на сетку.
ent-ComputerMessageBoard = консоль доски объявлений
    .desc = Используется для связи по всему Сектору.
ent-ComputerStationModification = консоль модификации фракции
    .desc = Используется для структурирования назначений и доступа.
ent-ComputerIdPrinter = консоль принтера ID-карт
    .desc = Печатает сменные ID-карты.
ent-ComputerInvoicePrinter = консоль принтера счетов
    .desc = Печатает подлежащие оплате счета.
